 ...
 * 
 * Deadlock - thread waiting on a condition that will never happen
 * Aging: prevents starvation / allows lower priority processes to not wait forever
 * We lock when changing elements on the ready_q or when accessing the current[] array
 * Process may be stalled due to an IO call
 * 
...
void enqueue(queue_t *queue, pcb_t *process)
{
    ...
    if (is_empty(queue)) {
        ...
        if (priority_with_age(get_current_time(), process) > priority_with_age(get_current_time(), queue->head)) {
            process->next = queue->head;
            queue->head = process;
        } else {
            while(curr->next != NULL && priority_with_age(get_current_time(), curr->next) < priority_with_age(get_current_time(), process) ) {
                curr = curr->next;
            }
...
{
    /* FIX ME */ // DONE
    pcb_t* nextP = dequeue(rq);
    if(nextP != NULL) {
        nextP->state = PROCESS_RUNNING;
    }
    ...
    if(scheduler_algorithm == PA) {
        ...
        for (unsigned int i = 0; i < cpu_count; i++) {
            if (current[i] == NULL) {
                pthread_mutex_unlock(&current_mutex);
                return;
            } else if (priority_with_age(get_current_time(), current[i]) < priority_with_age(get_current_time(), process) && priority_with_age(get_current_time(), current[i]) < highestPriority) {
                cpu = i;
                foundHighterPriority = 1;
                highestPriority = priority_with_age(get_current_time(), current[i]);
            }
        }
        pthread_mutex_unlock(&current_mutex);
        if(foundHighterPriority == 1) {
            force_preempt(cpu);
        }
    } 
}
